...
+(WKLoadingView*)showIn:(UIView*)view{
    [self hideIn:view];
    WKLoadingView *hud = [[WKLoadingView alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+(WKLoadingView *)hideIn:(UIView *)view{
    WKLoadingView *hud = nil;
    for (WKLoadingView *subView in view.subviews) {
        if ([subView isKindOfClass:[WKLoadingView class]]) {
            [subView hide];
