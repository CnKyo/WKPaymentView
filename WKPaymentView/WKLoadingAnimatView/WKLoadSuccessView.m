...
@implementation WKLoadSuccessView
{
...
+ (WKLoadSuccessView*)showIn:(UIView*)view {
    [self hideIn:view];
    WKLoadSuccessView *hud = [[WKLoadSuccessView alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+ (WKLoadSuccessView *)hideIn:(UIView *)view {
    WKLoadSuccessView *hud = nil;
    for (WKLoadSuccessView *subView in view.subviews) {
        if ([subView isKindOfClass:[WKLoadSuccessView class]]) {
            [subView hide];
